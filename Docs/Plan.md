...
- [x] Create `src/inputs/utilities/rumcap/trace-synthesizer.js`: `Capture` → viewer-targeted trace resources (µs timestamps, zero-based page clock). DevTools receives Chrome-trace-format JSON (`getPageResource(pageId, 'trace')`, gzipped for `loadFromFile`); Perfetto receives native TrackEvent protobuf (`getPageResource(pageId, 'perfetto-trace')`) so request tracks, custom tracks, and SQL args use Perfetto's native model instead of Chrome JSON importer quirks. Synthesize:
  - **Scaffolding:** `__metadata` `process_name` ("Renderer") / `thread_name` (`CrRendererMain` + a second thread for the profiler), `TracingStartedInBrowser` with a frames array (processId/frame/url from the navigation) — required by DevTools' MetaHandler — and `navigationStart` with `documentLoaderURL`, `isLoadingMainFrame`, `isOutermostMainFrame`, `navigationId`.
  - **DevTools network track:** per entry `ResourceWillSendRequest`/`ResourceSendRequest` (synthetic `requestId`, url, priority, method), `ResourceReceiveResponse` (statusCode, mimeType, the ResourceTiming-derived `timing` block), `ResourceFinish` (encodedDataLength). Honor `DEVTOOLS_REQUIRED_ARG_PATHS`: every `Resource*` event must carry `data.requestId`.
  ...
  - **User Timing:** `blink.user_timing` instant (marks) and async b/e (measures) with `detail` passed through — React 19 / Vue `detail.devtools` payloads then light up DevTools' extensibility tracks with zero extra work.
  - **customEvents:** DevTools namespaces become `blink.user_timing` measures carrying a synthesized `detail.devtools = { track: namespace }`; Perfetto namespaces become native child tracks under `Custom Events`.
  - **Profile:** the folded call tree maps 1:1 to complete `X` events for DevTools and native TrackEvent slices for Perfetto (name = frame function name, nested by depth/pre-order). DevTools uses the dedicated "JS Self-Profiling" thread; native Perfetto uses a plain `JS Self-Profiling` child track so the UI does not append a tid. No fake per-sample resynthesis; the 1 ms duration grid is what the wire stores.
...
- [ ] Manual pass over the rumcap sample corpus in both embedded viewers. *(Status: automated structural validation now covers both DevTools Chrome JSON and native Perfetto protobuf in `tests/inputs/rumcap-trace.test.js`; the interactive browser pass is still pending.)* Follow-up candidates after visual review: native Perfetto counter tracks for CLS/INP and UI-specific track ordering/colors.
