...
- [x] User Timing: marks + measures → `page._user_timing` (renderer's vertical-mark shape); keep `detail` for the trace synthesizer.
...
- [x] Create `src/inputs/utilities/rumcap/trace-synthesizer.js`: `Capture` → Chrome-trace-format JSON events (µs timestamps, `ts = round(startTime × 1000)` on a zero base). One synthesizer feeds both embedded viewers — Perfetto UI ingests Chrome JSON natively and DevTools' `loadFromFile` path already accepts gzipped JSON. Synthesize:
  ...
  - **Long tasks:** ≥50 ms `RunTask` X events on `CrRendererMain` (`toplevel` cat) — the shape DevTools derives its long-task candystriping from.
  ...
  - **customEvents:** each namespace becomes its own track — emit as `blink.user_timing` measures carrying a synthesized `detail.devtools = { track: namespace }` (DevTools renders named custom tracks; nesting from `depth` → stacked async events). In Perfetto the same events render as async tracks per namespace.
  - **Profile:** the folded call tree maps 1:1 to complete `X` events (name = frame function name, nested by depth/pre-order) on the dedicated "JS Self-Profiling" thread — both viewers render that as a flame chart naturally, and it stays honest (no fake per-sample resynthesis; 1 ms duration grid is what the wire stores).
...
- [x] `getPageResource(pageId, 'trace')` route for rumcap inputs (gated per the previous task): lazily synthesize from the retained `Capture` (dynamic-import the synthesizer module so it stays out of the base bundle), gzip via `CompressionStream('gzip')`, cache per page. (All target runtimes ship `CompressionStream`; if it's ever absent, an uncompressed `application/json` Blob is a valid fallback for both viewers.) This single hook gates + feeds both the Perfetto and DevTools tabs (existing viewer logic). Verify DevTools load via the established `panel.loadFromFile(new cw.File(...))` path and Perfetto via the JSON blob path.
...
- [ ] Evaluate (manual pass over the sample corpus in both viewers) whether Chrome-JSON rendering in Perfetto is good enough for custom-event tracks and vitals. *(Status: automated structural validation of the synthesized traces is done — `tests/inputs/rumcap-trace.test.js` — but the interactive browser pass over both embedded viewers is still pending.)* If not, follow-up (explicitly deferred, not in this phase): a native Perfetto `TracePacket`/`TrackEvent` protobuf emitter — we already have deep TrackEvent wire knowledge from the decoder — enabling proper named tracks and a CLS/interaction counter track.
