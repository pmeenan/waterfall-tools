...
- [ ] Add the `rumcap` npm package as a runtime dependency (Apache-2.0 ✓, zero-dep, ESM, isomorphic, `sideEffects: false`). Only `rumcap/decode` (`unpack`, `sniff`, `checkConsistency`) is imported so the encoder tree-shakes away. Peer-folder source at `../rumcap` is the reference; consume the published package, not a file link, so CI installs cleanly.
...
- [ ] Profile → per-request JS overlays: frames carry `resourceId` → script URL; attribute top-level slice intervals to entries by matching the frame's resource URL against `_full_url` (first-match-wins, same `$used` de-dup as the other parsers) → `entry._js_timing`.
- [ ] Attach non-rendered capture context for the details UI: `page._rumcapManifest` (per-stream status/loss/provenance), `page._rumcapEnvironment`, `page._rumcapMetadata`, plus `errors`, `visibility`, `loaf`, `interactions`, `elementTiming`, and `customEvents` under `_`-prefixed page fields so `getHar()` round-trips them. Document every new `_` field in `Docs/Extended-HAR-Schema.md` **and** the JSDoc types in `src/core/har-types.js` (both are the schema of record per AGENTS.md).
- [ ] CLI wrapper `src/inputs/cli/rumcap.js`, Conductor/orchestrator registration, `bin/waterfall-tools.js` auto-detect. Verify eslint `files` globs cover the new paths; run `npm run lint`.
...
- [ ] `getPageResource(pageId, 'trace')` route for rumcap inputs (gated per the previous task): lazily synthesize from the retained `Capture` (dynamic-import the synthesizer module so it stays out of the base bundle), gzip via `CompressionStream('gzip')`, cache per page. (All target runtimes ship `CompressionStream`; if it's ever absent, an uncompressed `application/json` Blob is a valid fallback for both viewers.) This single hook gates + feeds both the Perfetto and DevTools tabs (existing viewer logic). Verify DevTools load via the established `panel.loadFromFile(new cw.File(...))` path and Perfetto via the JSON blob path.
- [ ] Tests for the synthesizer: structural assertions that every emitted event satisfies the DevTools required-arg paths, timestamps are monotonic non-negative µs, B/E (async) pairs balance, and profile X events nest without overlap violations within a depth chain.
- [ ] Evaluate (manual pass over the sample corpus in both viewers) whether Chrome-JSON rendering in Perfetto is good enough for custom-event tracks and vitals. If not, follow-up (explicitly deferred, not in this phase): a native Perfetto `TracePacket`/`TrackEvent` protobuf emitter — we already have deep TrackEvent wire knowledge from the decoder — enabling proper named tracks and a CLS/interaction counter track.

### Phase 10d: Viewer & docs wrap-up
- [ ] Landing page copy + feature list: add rumcap/`.rcap` to supported formats (AGENTS.md viewer contract).
- [ ] Show INP in the viewer summary grid (`viewer.js#L703` `metricsGroup`): add an "Interaction to Next Paint" row from `pageData._InteractionToNextPaint` near LCP/CLS, with `getRating(..., [200, 500])` per the web-vitals thresholds. Rows with undefined values already render as absent, so the addition is inert for every other format.
- [ ] Request details tab sanity pass: rcap entries have no headers/bodies — confirm the tab renders gracefully (timings, server-timing, raw JSON) and the capture-level extras (`_rumcapEnvironment`, errors, manifest) are reachable from the page-level details.
- [ ] `README.md` formats table + usage; `Docs/Architecture.md`; `Docs/Extended-HAR-Schema.md` + `src/core/har-types.js` JSDoc (new `_` fields); `AGENTS.md` parser-specific rules section for rumcap (clock model, absence semantics, tri-state `_longTasks`, trace-synthesis contract, Worker lockstep reminder).
