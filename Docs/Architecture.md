...
qlog is the IETF structured logging format for QUIC and HTTP/3 ([main schema](https://quicwg.org/qlog/draft-ietf-quic-qlog-main-schema.html), [QUIC events](https://quicwg.org/qlog/draft-ietf-quic-qlog-quic-events.html), [HTTP/3 events](https://quicwg.org/qlog/draft-ietf-quic-qlog-h3-events.html) — all still Internet-Drafts). One qlog file describes one QUIC connection, so a page load typically produces several files (one per origin). Support is **beta**: the real-world corpus is thin (curl/ngtcp2 and aioquic client captures); server-vantage captures are implemented but unexercised pending sample files (GitHub issue #12). The pipeline:

1. **Sniff** — content-based, never extension-based, mirrored between `src/inputs/orchestrator.js` and `cloudflare-worker/worker.js`. JSON-SEQ `.sqlog` (RFC 7464): first meaningful byte is the `0x1E` record separator plus the `"qlog_version"` token within the sniff window. Plain JSON `.qlog`: the `"qlog_version"` token checked ahead of the chrome-trace/HAR sniffs. An outer user-applied gzip layer is detected by magic bytes and inflated first.
...
3. **Clock resolution** (per trace) — a truthy `reference_time` (number, numeric string, or `{wall_clock_time, epoch}` object) anchors the trace to wall clock; with no anchor but event times deep in epoch-ms range (> 1e12, e.g. aioquic) the times themselves are the anchor; otherwise the trace is unanchored (curl's `reference_time: 0`) and the page epoch is synthesized from `Date.now()` with `page._clockSynthesized = true` — never a 1970 date. `time_format: "delta"` times are cumulative-summed.
4. **Stream assembly** (`src/inputs/qlog.js`) — events replay into a per-connection model: per-stream tx/rx byte ledgers from packet STREAM frames, handshake bounds, server→client packet sizes (for a tcpdump-style 100 ms sliding-window `_bwDown` estimate), and rtt/cwnd samples from `recovery:metrics_updated`. One HAR entry per client-initiated bidirectional stream (id % 4 === 0) with request/payload evidence; `http3:stream_type_set` excludes streams declared with a non-`request` role. Vantage-point aware — server captures flip packet/frame direction semantics.
5. **Extended HAR** — two fidelity modes. *Rich* (HTTP/3 HEADERS events present, e.g. aioquic): real method/URL/status/headers/MIME type, plus `_priority` from the RFC 9218 `priority: u=N` request header. *Degraded* (transport-only, e.g. curl/ngtcp2, whose headers stay QPACK-encoded inside opaque STREAM bytes): synthetic `https://connection-<odcid8>/stream-<id>` URLs, `GET`, assumed status `200` with `_statusAssumed` (rumcap precedent). Timings use the renderer's absolute-timings fields; the QUIC handshake is TLS-integral so `_connect_*` and `_ssl_*` share one span, carried by the earliest request on each connection. No `_dns_*` is ever emitted — qlog has no DNS events and missing is better than wrong. The connection ODCID lands on `entry.connection` so Connection View groups streams per QUIC connection, and per-connection context (`page._qlogTraces`) plus capped rtt/cwnd series (`page._qlogMetrics`) ride along for the details UI.
