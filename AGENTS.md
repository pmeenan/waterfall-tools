...
- Long tasks: top-level (parent-less) events ≥ 50 ms on any main-thread candidate, coalesced into `page._longTasks = [[ms_start, ms_end], ...]` so the Long Tasks band has proper data instead of falling back to the legacy flat `_mainThreadEvents` path.
